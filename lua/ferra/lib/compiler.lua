local path_sep = require("ferra").path_sep
...
function M.compile(flavour)
	local theme = require("ferra.lib.mapper").apply(flavour)
	local lines = {
		string.format(
			[[
...
local h = vim.api.nvim_set_hl]]
	...
	if vim.g.ferra_debug then -- Debugging purpose
	...
	if not f then
		local err_path = (path_sep == "/" and "/tmp" or os.getenv "TMP") .. "/ferra_error.lua"
		print(string.format(
			[[Ferra (error): Most likely some mistake made in your ferra config
