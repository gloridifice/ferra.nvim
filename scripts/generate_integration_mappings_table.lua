...
local ferra_path = vim.fn.getcwd() -- get to the root directory of the plugin
local lua_path = ferra_path .. "/lua"

package.path = package.path .. string.format(";%s/?.lua", lua_path) .. string.format(";%s/?/init.lua", lua_path)

local integrations_path = lua_path .. "/ferra/groups/integrations/"
local target_path = lua_path .. "/ferra/utils/integration_mappings.lua"

---@return table mappings table holding plugin to ferra name
local function create_table()
	local mappings = vim.iter(vim.fs.dir(integrations_path)):fold({}, function(m, filename, _)
		...
		local ok, mod = pcall(require, "ferra.groups.integrations." .. filename)
		...
		local plugin_name = require("ferra.lib.detect_integrations").parse_url(plugin_url)
